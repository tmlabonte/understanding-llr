...
datasets = {
    "celeba": "celeba_resnet_layerwise.pkl",
    "waterbirds" : "waterbirds_resnet_layerwise.pkl"
}
colors = {
    "none": "C0",     
...
def process_values(results, erm_results, llr_results, retrain_key_erm, retrain_key_llr, dataset, seeds):
    ...
    for s in seeds:
        try:
            # WGA Values
            wga_layerwise_values.append(llr_results[s][50][retrain_key_llr]["erm"][test_epoch]["test_wga"])
            wga_llr_values.append(llr_results[s][50][retrain_key_llr]["llr"][retrain_key_llr][test_epoch]["test_wga"])
            
            # Accessing ERM Results
            wga_erm_values.append(erm_results[s][50][retrain_key_erm]["erm"][test_epoch]["test_wga"])
            
            # AA Values
            aa_layerwise_values.append(llr_results[s][50][retrain_key_llr]["erm"][test_epoch]["test_aa"])
            aa_llr_values.append(llr_results[s][50][retrain_key_llr]["llr"][retrain_key_llr][test_epoch]["test_aa"])
            
            # Accessing ERM AA Results
            aa_erm_values.append(erm_results[s][50][retrain_key_erm]["erm"][test_epoch]["test_aa"])
...
def process_layerwise_models(dataset, erm_filepath, llr_filepath, categories):
    ...
    plot_layerwise_bars(data_wga, errors_wga, categories, "Test WGA", f"Layerwise Resnet Comparison - {dataset.capitalize()} (WGA)", f"{dataset}_layerwise_resnet_comparison_wga.png", ylim = (0.3, 0.85))
    plot_layerwise_bars(data_aa, errors_aa, categories, "Test AA", f"Layerwise Resnet Comparison - {dataset.capitalize()} (AA)", f"{dataset}_layerwise_resnet_comparison_aa.png", ylim = (0.5,1))

# Process each dataset separately
process_layerwise_models("celeba", "/home/xzhang941/understanding-llr/celeba_resnet.pkl", "/home/xzhang941/understanding-llr/celeba_resnet_layerwise.pkl", ["none", "subsetting", "upsampling"])
process_layerwise_models("waterbirds", "/home/xzhang941/understanding-llr/waterbirds_resnet.pkl", "/home/xzhang941/understanding-llr/waterbirds_resnet_layerwise.pkl", ["none", "subsetting", "upsampling"])
