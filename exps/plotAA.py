...
def plot_subplots(data, errors, categories, ylabel, title, output_file, dataset):
    ...
    for i, erm in enumerate(balance_erm):
        ...
        y_max = min(1.0, max([m + s for m, s in zip(means, stds)]) + 0.05)
        ...
        ax.set_ylim(y_min, y_max)  # Set dynamic y-axis limits with ±std and ±0.05 margin, capped at 1.0
...
def process_dataset(dataset, categories, output_subplots, metric):
    ...
    for erm in balance_retrain:
        ...
        for retrain in balance_retrain:
            ...
            for s in seeds:
                try:
                    if metric == "train_aa":
                        metric_values.append(results[s][50][erm]["llr"][retrain][epoch_for_dataset]["train_aa"])
                        metric_values_no_llr.append(results[s][50][erm]["erm"][epoch_for_dataset]["train_aa"])
                    elif metric == "wca":
                        metric_values.append(results[s][50][erm]["llr"][retrain][epoch_for_dataset]["test_acc_by_class"])
                        metric_values_no_llr.append(results[s][50][erm]["erm"][epoch_for_dataset]["test_acc_by_class"])
                except KeyError:
    ...
    plot_subplots(combined_data, combined_errors, categories, "Train AA" if metric == "train_aa" else "WCA", 
                  f"{dataset.replace('_', ' ').capitalize()} {'TrainAA' if metric == 'train_aa' else 'WCA'}", 
                  output_subplots, dataset)

def main():
    if len(sys.argv) != 2:
        print("Usage: python script.py <metric>")
        print("Where <metric> is either 'train_aa' or 'wca'")
        return

    metric = sys.argv[1]
    if metric not in ['train_aa', 'wca']:
        print("Invalid metric. Use 'train_aa' or 'wca'.")
        return

    process_dataset("celeba_resnet", ["none", "upsampling", "subsetting"], 
                    f"celeba_Resnet {'TrainAA' if metric == 'train_aa' else 'WCA'}", metric)
    process_dataset("waterbirds_resnet", ["none", "upsampling", "subsetting"], 
                    f"Waterbirds_Resnet {'TrainAA' if metric == 'train_aa' else 'WCA'}", metric)
