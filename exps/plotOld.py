...
def plot_subplots(data, errors, categories, ylabel, title, output_file, dataset):
    ...
    for i, erm in enumerate(balance_erm):
        ...
        if dataset == "civilcomments_bert" and "WGA" in title:
            ...
            y_min, y_max = 0.50, 0.9  # Customize this range as per your requirements
            ...
            for s in seeds:
                try:
                    ...
                    if metric == "train_aa":
                        ...
                        metric_value = results[s][model_key][erm]["llr"][retrain][epoch_for_dataset]["test_wca"]
                        metric_value_no_llr = results[s][model_key][erm]["erm"][epoch_for_dataset]["test_wca"]
                    elif metric == "wga":
...
def plot_scatterplot(test_wga_data, test_aa_data, colors_list, labels_list, title, output_file):
    ...
    pearson_corr, _ = pearsonr(test_aa_data, test_wga_data)
    print(f"Pearson correlation between Test WGA and Test AA: {pearson_corr:.3f}")
...
def plot_comparison_scatterplot(x_data, y_data, labels_list, title, output_file):
    ...
    print(f"Pearson correlation between ConvNext and ResNet: {pearson_corr:.3f}")
    ax.set_title(f"{title} (Pearson: {pearson_corr:.3f})")
...
def plot_test_aa_vs_test_wca(dataset, categories, metric):
    try:
        with open(f"{dataset}.pkl", "rb") as f:
            results = pickle.load(f)
        print(f"Successfully loaded {dataset}.pkl")
        ...
        for retrain in balance_retrain:
            ...
            for s in seeds:
                try:
                    # Collect LLR data for each seed
                    aa_values_llr.append(results[s][model_key][erm]["llr"][retrain][epoch_for_dataset]["test_aa"])
                    wca_values_llr.append(results[s][model_key][erm]["llr"][retrain][epoch_for_dataset]["test_wca"])
                    
                    # Collect ERM data for each seed
                    aa_values_erm.append(results[s][model_key][erm]["erm"][epoch_for_dataset]["test_aa"])
                    wca_values_erm.append(results[s][model_key][erm]["erm"][epoch_for_dataset]["test_wca"])

                except KeyError:
                    print(f"Missing data for seed {s}, model {model_key}, {erm}.")
                    aa_values_llr.append(np.nan)
                    wca_values_llr.append(np.nan)
                    aa_values_erm.append(np.nan)
                    wca_values_erm.append(np.nan)

            # Compute average across the 3 seeds for LLR
            aa_mean_llr = np.nanmean(aa_values_llr)
            wca_mean_llr = np.nanmean(wca_values_llr)

            # Compute average across the 3 seeds for ERM
            aa_mean_erm = np.nanmean(aa_values_erm)
            wca_mean_erm = np.nanmean(wca_values_erm)

            # Collect the averaged LLR data for scatterplot
            test_aa_data.append(aa_mean_llr)
            test_wca_data.append(wca_mean_llr)
            colors_list.append(colors[erm])
            labels_list.append(f"{labels[erm]} / {labels[retrain]}")

            # Collect the averaged ERM data for scatterplot
            test_aa_data.append(aa_mean_erm)
            test_wca_data.append(wca_mean_erm)
            colors_list.append('gray')  # Different color for ERM
            labels_list.append(f"{labels[erm]}")  # Updated label for ERM

    # Call the plot function with the correct arguments for test_aa vs test_wca
    plot_scatterplot_aa_wca(test_aa_data, test_wca_data, colors_list, labels_list, f"{dataset} Test AA vs WCA", f"{dataset}_test_aa_vs_wca_scatterplot.png",dataset)

def plot_scatterplot_aa_wca(test_aa_data, test_wca_data, colors_list, labels_list, title, output_file, dataset):
    fig, ax = plt.subplots(figsize=(15, 9))
        # Default axis limits if the dataset is unknown
    ax.set_xlim(0.5, 1.0)
    ax.set_ylim(0.5, 1.0)
    
    # Plot each point and label
    for i in range(len(test_aa_data)):
        ax.scatter(test_aa_data[i], test_wca_data[i], color=colors_list[i], label=labels_list[i], s=180)
        ax.annotate(f'{labels_list[i]}', (test_aa_data[i], test_wca_data[i]), textcoords="offset points", 
                    xytext=(0, 8), ha='center', fontsize=6)

    pearson_corr, _ = pearsonr(test_aa_data, test_wca_data)
    print(f"Pearson correlation between Test AA and Test WCA: {pearson_corr:.3f}")
    ax.set_title(f"{title} (Pearson: {pearson_corr:.3f})")
    ax.set_xlabel('Test AA (Average Accuracy)')
    ax.set_ylabel('Test WCA (Worst-Class Accuracy)')
...
def plot_test_wca_vs_test_wga(dataset, categories, metric):
    try:
        with open(f"{dataset}.pkl", "rb") as f:
            results = pickle.load(f)
        print(f"Successfully loaded {dataset}.pkl")
    ...
    # Call the plot function with the correct arguments for test_wca vs test_wga
    plot_scatterplot_wca_wga(test_wca_data, test_wga_data, colors_list, labels_list, f"{dataset} Test WCA vs WGA", f"{dataset}_test_wca_vs_wga_scatterplot.png",dataset)

def plot_scatterplot_wca_wga(test_wca_data, test_wga_data, colors_list, labels_list, title, output_file,dataset):
    fig, ax = plt.subplots(figsize=(15, 9))
    ax.set_xlim(0.3, 1.0)
    ax.set_ylim(0.3, 1.0)

    # Plot each point and label
    for i in range(len(test_wca_data)):
        ax.scatter(test_wca_data[i], test_wga_data[i], color=colors_list[i], label=labels_list[i], s=180)
        ax.annotate(f'{labels_list[i]}', (test_wca_data[i], test_wga_data[i]), textcoords="offset points", 
                    xytext=(0, 8), ha='center', fontsize=6)

    pearson_corr, _ = pearsonr(test_wca_data, test_wga_data)
    print(f"Pearson correlation between Test WCA and Test WGA: {pearson_corr:.3f}")
    ax.set_title(f"{title} (Pearson: {pearson_corr:.3f})")
    ax.set_xlabel('Test WCA (Worst-Class Accuracy)')
...
def main(args):
    ...
    elif metric == 'aa_vs_wca':
        plot_test_aa_vs_test_wca("celeba_resnet", ["none", "upsampling", "subsetting"],metric)
        plot_test_aa_vs_test_wca("celeba_convnextv2", ["none", "upsampling", "subsetting"],metric)
        plot_test_aa_vs_test_wca("waterbirds_resnet", ["none", "upsampling", "subsetting"],metric)
        plot_test_aa_vs_test_wca("waterbirds_convnextv2", ["none", "upsampling", "subsetting"],metric)
        plot_test_aa_vs_test_wca("civilcomments_bert", ["none", "upsampling", "subsetting"],metric)

    elif metric == 'wca_vs_wga':
        plot_test_wca_vs_test_wga("celeba_resnet", ["none", "upsampling", "subsetting"], metric)
        plot_test_wca_vs_test_wga("celeba_convnextv2", ["none", "upsampling", "subsetting"], metric)
        plot_test_wca_vs_test_wga("waterbirds_resnet", ["none", "upsampling", "subsetting"], metric)
        plot_test_wca_vs_test_wga("waterbirds_convnextv2", ["none", "upsampling", "subsetting"],metric)
        plot_test_wca_vs_test_wga("civilcomments_bert", ["none", "upsampling", "subsetting"], metric)
